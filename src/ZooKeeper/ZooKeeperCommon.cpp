...
ZooKeeperResponsePtr ZooKeeperRemoveRecursiveRequest::makeResponse() const
{
    return std::make_shared<ZooKeeperRemoveRecursiveResponse>();
}

void ZooKeeperCheckStatRequest::writeImpl(WriteBuffer & out) const
{
    Coordination::write(path, out);
    Coordination::write(version, out);
...
{
    Coordination::write(path, out);
    Coordination::write(max_entries, out);
}

void ZooKeeperListRecursiveRequest::readImpl(ReadBuffer & in)
{
    Coordination::read(path, in);
    Coordination::read(max_entries, in);
}
...
ZooKeeperMultiRequest::ZooKeeperMultiRequest(const Requests & generic_requests, const ACLs & default_acls)
{
    ...
    for (const auto & generic_request : generic_requests)
    {
        ...
        {
            checkOperationType(OperationType::Write);
        ...
        {
            checkOperationType(OperationType::Read);
...
{
    Coordination::read(relative_zxid, in);
...
void ZooKeeperNewSessionRequest::writeImpl(WriteBuffer & out) const
{
...
void ZooKeeperNewSessionRequest::readImpl(ReadBuffer & in)
{
...
Coordination::ZooKeeperResponsePtr ZooKeeperNewSessionRequest::makeResponse() const
{
    auto response = std::make_shared<ZooKeeperNewSessionResponse>();
...
void ZooKeeperNewSessionResponse::readImpl(ReadBuffer & in)
{
    Coordination::read(internal_id, in);
...
{
    auto response = std::make_shared<ZooKeeperUpdateSessionResponse>();
...
void registerZooKeeperRequest(ZooKeeperRequestFactory & factory)
{
    factory.registerRequest(num, []
    {
        ...
        if constexpr (num == OpNum::MultiRead)
            ...
            res->list_with_stats_and_data = true;
...
ZooKeeperRequestFactory::ZooKeeperRequestFactory()
{
    ...
    registerZooKeeperRequest<OpNum::NewSession, ZooKeeperNewSessionRequest>(*this);
    ...
    registerZooKeeperRequest<OpNum::CheckStat, ZooKeeperCheckStatRequest>(*this);
    registerZooKeeperRequest<OpNum::TryRemove, ZooKeeperRemoveRequest>(*this);
    registerZooKeeperRequest<OpNum::FilteredListWithStatsAndData, ZooKeeperFilteredListRequest>(*this);
